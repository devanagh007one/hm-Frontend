...
export const handleLogout = () => async (dispatch) => {
    try {
        ...
        const trackingResponse = await fetch(`${process.env.REACT_APP_STATIC_API_URL}/api/user/track-availability`, {
            method: 'POST',
            credentials: 'include',
            headers: {
        ...
        if (trackingData.message === 'Availability tracked successfully') {
            ...
            const logoutResponse = await fetch(`${process.env.REACT_APP_STATIC_API_URL}/api/auth/logout`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${authToken}`,
            ...
            if (logoutData.message === 'Logged out successfully') {
                ...
                dispatch(showNotification('Failed to log out', 'error')); // Dispatch error notification
            }
        } else {
        ...
        dispatch(showNotification(`Logout process failed: ${error.message}`, 'error')); // Dispatch error notification
    }
};
