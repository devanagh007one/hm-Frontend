...
import { forgetPassword, verifyOtp, resetPassword } from '../redux/actions/ForgetAction';
...
const ForgetPassword = () => {
  ...
  const { loading, success, error, token, resetSuccess } = useSelector((state) => state.forgetPassword);
  ...
  const handlePasswordChange = (e) => {
    ...
    if (password === confirmPassword) {
      ...
      dispatch(showNotification('Password and confirm password must match.', 'error'));
    }
  };
    ...
    <div className='flex flex-col gap-5 w-3/5'>
      {showPasswordSection ? (
        ...
        <div className='password-section flex flex-col w-full items-center gap-4 slide-in-from-left'>
          <div className='text-lg subpixel-antialiased'>Enter your new password.</div>
          <input
            type='password'
        ...
        <div className='otp flex flex-col w-full items-center gap-4 slide-in-from-left'>
          <div className='text-lg subpixel-antialiased'>Enter the 6-digit OTP received on your mobile.</div>
          <div className='flex space-x-2'>
            {otp.map((data, index) => (
              <input
                className='shadow-inner h-12 w-12 text-lg text-center rounded-lg	 custom-input mb-5'
                type='text'
                name='otp'
                maxLength='1'
                key={index}
