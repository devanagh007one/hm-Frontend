...
import { fetchAllUsers, createUser, editUserProfile } from "../redux/actions/alluserGet.js";
...
const EditUser = ({ record }) => {
    ...
    useEffect(() => {
        if (record) {
            setFormData({
                ...
                goals: record.goals || "",
                dateOfAniversary: record.dateOfAniversary || "",
    ...
    const handleSubmit = (e) => {
        ...
        formData.append("employeeCount", formData.employeeCount);
        ...
        formData.append("dateOfAniversary", formData.dateOfAniversary);
        formData.append("relationShipStatus", formData.relationShipStatus);
        formData.append("bloodGroups", formData.bloodGroups);
        formData.append("childCount", formData.childCount);
        // Dispatch the edit action
        dispatch(editUserProfile(record._id, formData))
            .then(() => {
                dispatch(showNotification("User profile updated successfully", "success"));
                setShowPopup(false);
            })
            .catch((error) => {
                console.error(error);
        ...
        <>
            ...
            >
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path opacity="0.16" d="M4.16634 13.3333L3.33301 16.6667L6.66634 15.8333L14.9997 7.5L12.4997 5L4.16634 13.3333Z" fill="#C7C7C7" />
                    <path d="M12.4997 5.00007L14.9997 7.50007M10.833 16.6667H17.4997M4.16634 13.3334L3.33301 16.6667L6.66634 15.8334L16.3213 6.17841C16.6338 5.86586 16.8093 5.44201 16.8093 5.00007C16.8093 4.55813 16.6338 4.13429 16.3213 3.82174L16.178 3.67841C15.8655 3.36596 15.4416 3.19043 14.9997 3.19043C14.5577 3.19043 14.1339 3.36596 13.8213 3.67841L4.16634 13.3334Z" stroke="#C7C7C7" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
                            ...
                            <div className="flex gap-4">
                                <div className="flex flex-col w-1/2">
                                    ...
                                    >
                                        ...
                                        {/* Conditionally show options based on gender */}
                                        {(formData.gender === "Male" || formData.gender === "male") ? (
                                            ...
                                            // If gender is not male, show different options or the same options
                                            <>
                                                <option value="single">Single</option>
                                                <option value="married">Married</option>
