...
const ImageEditor = () => {
  ...
  const handleApply = async () => {
    ...
    img.onload = async () => {
      ...
      // Calculate scaling to fit the image within the circle
      const scale =
        Math.max(targetSize / img.width, targetSize / img.height) * zoom;

      // Calculate position to center the image with vertical offset
      const x = (targetSize - img.width * scale) / 2;
      const y = (targetSize - img.height * scale) / 2 + verticalOffset;
                      ...
                      <img
                        ...
                        style={{
                          transform: `scale(${zoom}) rotate(${
                            rotation + tilt
                          }deg) translateY(${verticalOffset}px)`,
                        }}
                      />
                    </div>
                  </section>
