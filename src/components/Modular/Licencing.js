...
import {
  ...
  toggleLicenseStatus,
  changeLicenseType,
} from "../../redux/actions/allLicensingGet.js";
...
const UserManagement = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    // Deduplicate based on organisationName, mobile, and numberOfLicence
    const seen = new Set();
    filtered = filtered.filter((item) => {
      const key = `${item.organisationName}-${item.mobile}-${item.numberOfLicence}`;
  ...
  const handleDownload = () => {
    const selectedRows = filteredData
      ...
      .map(
        ({ _id, __v, passwordChangedAt, macAddresses, editLogs, ...row }) =>
          row,
      );
    ...
    if (selectedFormat === ".pdf") {
      ...
      XLSX.writeFile(
        workbook,
        `HappMe-data.${selectedFormat.replace(".", "")}`,
      );
    } else {
  ...
  const handleToggle = async (record) => {
    ...
    const newPeriod =
      record.period === "Subscription" ? "Trial" : "Subscription";
    ...
    try {
      ...
      dispatch(
        showNotification(
          `Changed ${record.organisationName} to ${newPeriod}`,
          "success",
        ),
      );
  ...
  const handleToggleStatus = async (licenseId, currentStatus) => {
    try {
      ...
      console.log(
        `${currentStatus === "Active" ? "Deactivated" : "Activated"} license with ID: ${licenseId}`,
      );
      ...
      title: (
        <span className="flex items-center">
          <SvgIcon
            onClick={() =>
              setFilter((prev) =>
                prev === "licenceMinToMax" ? "" : "licenceMinToMax",
              )
            }
          />
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const isSubscription =
          toggleStates[record._id] !== undefined
            ? toggleStates[record._id]
            : record.period === "Subscription";
  ...
  return (
    <>
      <div
        className={`flex justify-center content-center rounded-lg max-h-14 absolute right-[300px] top-[25px] ${darkMode ? "bg-zinc-800 text-zinc-300" : "bg-slate-100 text-black"} transition-colors duration-300`}
      >
        <input
      ...
      <div className="flex flex-col  p-5  thescreanhe">
        {/* Page Header */}
        <div
          className={`flex justify-between items-center mb-6 px-12 py-3 rounded-lg mt- ${darkMode ? "bg-[#333333]" : "bg-[#f1f5f9]"}`}
        >
          ...
          <div className="flex gap-6">
            ...
            {showImportForm && (
              <div
                className={`fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 ${darkMode ? "bg-[#333333]" : "bg-[#f1f5f9]"}`}
              >
                <div
                  className={`rounded-lg shadow-lg w-full max-w-xl p-8 relative ${darkMode ? "bg-[#333333] text-dark" : "bg-[#f1f5f9] text-dark"}`}
                >
        ...
        <div className="flex-grow overflow-x-auto h-11 px-8 theusertab">
          ...
          <div className="relative">
            ...
            {/* Table */}
            <div
              className={`table-container ${darkMode ? "bg-[#333333]" : "bg-[#f1f5f9]"} rounded-lg`}
            >
              <table
              ...
              >
                <thead
                  className={`custom-table ${darkMode ? "bg-[#18181b]" : "bg-[#f1f5f9]"}`}
                >
                  <tr
                    className={`rounded-custom ${darkMode ? "bg-[#18181b]" : "bg-[#f1f5f9]"}`}
                  >
            ...
            <div className="absolute left-[-20px] bottom-0 flex flex-col items-center justify h-full pt-[45px]">
              {paginatedData.map((_, index) => {
                ...
                return (
                  <span
                    key={absoluteIndex}
                    className="h-[58.5px] flex justify-center items-center"
                  >
                    <input
