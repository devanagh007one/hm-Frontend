...
import {
  ...
  updateLicense,
  deleteLicence,
} from "../../redux/actions/allLicensingGet";
import { showNotification } from "../../redux/actions/notificationActions";

const BarttForm = ({ data }) => {
  ...
  const handleDelete = async (data) => {
    ...
    localStorage.setItem("activeComponent", "Licensing");
    window.location.reload();
  };

  // Fixed: Separate keys for organization name and contact person name
  const fields = [
    { label: "Name of the Organization", key: "organisationName" }, // Changed from contactPersonName
    { label: "Industry", key: "industryType" },
    { label: "Organization Size", key: "organizationSize" },
    { label: "Contact Person Name", key: "contactPersonName" }, // This remains the same
  ...
  useEffect(() => {
    if (data) {
      setFormData({
        organisationName: data?.organisationName || "", // Added organisationName
        contactPersonName: data?.contactPersonName || "",
        industryType: data?.industryType || "",
        organizationSize: data?.organizationSize || "",
        contactPersonEmail: data?.contactPersonEmail || "",
        phoneNumber: data?.phoneNumber || "",
        usedLicenses: licenseData?.license?.usedLicenses || "",
    ...
    <>
      ...
      {showPopup && (
        <div className="popup-overlay">
          <div
            className={`rounded-lg shadow-lg w-[35%] max-w-3xl p-8 relative flex flex-col max-h-[90%] ${
                ...
                <div>
                  <input
                    ...
                    className={`ml-4 p-1 rounded-md ${
                      darkMode ? "bg-gray-700" : "bg-[#f1f5f9]"
                    }`}
                  />
                </div>
              )}
              ...
              <div className="space-y-3">
                {fields.map((field, index) => {
                  return (
                    ...
                    >
                      <span className="font-medium capitalize w-[180px]">
                        {field.label}:
                      </span>
