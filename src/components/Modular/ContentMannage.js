...
const ContentManagement = ({ data }) => {
  ...
  useEffect(() => {
    if (data?._id && challenges.length > 0) {
      const defaultChallenge = challenges.find(
        (challenge) => challenge._id === data._id
      );
  ...
  const fields = [
    {
      label: "Author",
      value: `${selectedModule?.uploaded_by?.firstName || "N/A"} ${
        selectedModule?.uploaded_by?.lastName || ""
      }`,
    },
    ...
    {
      label: "Track",
      value: selectedModule?.module?.tracks || data?.module?.tracks,
    },
    {
      label: "Challenges",
      value: selectedModule ? selectedModule.challengeName : "Not Found",
    },
              ...
              <div className="space-y-3">
                {fields.map((field, index) => (
                  ...
                  >
                    ...
                    <span className="ml-4 w-[80%] capitalize font-thin">
                      {field.value || "N/A"}
                    </span>
                  </div>
                ))}
