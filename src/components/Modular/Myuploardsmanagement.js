...
import { updateEventStatus, deleteEvent } from '../../redux/actions/alleventGet.js';
...
const Myuploardsmanagement = () => {
    ...
    const handleApprovalAction = (record, status) => {
        ...
        if (typeof typeOfEvent === "string" && typeOfEvent.trim() !== "") {
            ...
            dispatch(patchTheContent(_id, formattedStatus))
                ...
                .catch(() => {
                    dispatch(showNotification(`Failed to update content status to ${formattedStatus}`, "error"));
                });
        } else {
    ...
    const handleDeleteAction = (record) => {
        ...
        if (typeof typeOfEvent === "string" && typeOfEvent.trim() !== "") {
            ...
            dispatch(deleteChallenge(_id))
                .then(() => {
                    dispatch(showNotification(`Successfully deleted challenge`, "success"));
                    dispatch(fetchAllContent());
                })
                .catch(() => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => {
                const contentName = record.moduleName || record.challengeName || record.title;
        ...
        },
        {
            ...
            render: (isApproved, record) => {
                ...
                const status = statusMap[(isApproved || record.status)?.toLowerCase()] || "Inactive";
        ...
        },
        {
            ...
            key: "tracks",
            render: (text, record) => record.tracks || record.module?.tracks || record.typeOfEvent,
        },
        {
            title: (
                <div className="flex items-center">
