...
const EyeForm = ({ record }) => {
  ...
  useEffect(() => {
    ...
    setTeams(
      Object.entries(allTeams).map(([name, id]) => ({
        id,
        name,
        selected: id === savedSelectedTeam,
      }))
    );

    // Fetch user's current team when record changes
  }, [record]);

  // Function to fetch user's current team
  ...
  const handleTeamToggle = (id) => {
    // Don't allow toggling locked teams (except the user's current team)
    if (lockedTeams.includes(id) && id !== userCurrentTeam?.id) return;
  ...
  const handleAssignTeams = async () => {
    try {
      ...
      // Update the locked teams after assignment
              ...
              {teams.map((team) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="w-4 h-4 rounded-full border-2 border-[#F48567] flex items-center justify-center"
                      onClick={() => !isLocked && handleTeamToggle(team.id)}
                    >
                      {team.selected && (
                        <div className="w-2 h-2 rounded-full bg-[#F48567]" />
                      )}
