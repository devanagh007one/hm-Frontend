...
const EyeForm = ({ record }) => {
    ...
    const [showPopup, setShowPopup] = useState(false);
    const [selectedTeams, setSelectedTeams] = useState([]);
    const [selectedTeamLabels, setSelectedTeamLabels] = useState([]);
    const [lockedTeams, setLockedTeams] = useState([]); // Teams that can't be deselected
    ...
    useEffect(() => {
        if (record?.teams) {
            ...
            setSelectedTeams(record.teams);
            setSelectedTeamLabels(
                teamOptions.filter((team) => record.teams.includes(team.value)).map((team) => team.label)
            );
        }
    }, [record]);
    ...
    const handleTeamChange = (selectedValues) => {
        // Ensure locked teams stay selected
        const updatedSelection = [...new Set([...lockedTeams, ...selectedValues])];

        setSelectedTeams(updatedSelection);
        setSelectedTeamLabels(
            teamOptions.filter((team) => updatedSelection.includes(team.value)).map((team) => team.label)
        );
    };
    ...
    return (
        <>
            <div onClick={() => setShowPopup(true)} className="cursor-pointer flex flex-row">
            ...
            {showPopup && (
                <div className="popup-overlay asign-tem">
                    <div className={`rounded-xl border border-gray-600 shadow-lg w-[30%] overflow-y-auto p-8 relative flex flex-col max-h-[95%] ${darkMode ? 'bg-[#222222] text-white' : 'bg-white text-black'}`}>
                        <div className="flex justify-between items-center">
                            <h2 className="text-2xl mb-6">Select Teams</h2>
                            <svg className="cursor-pointer mt-1 mb-6" onClick={() => setShowPopup(false)} width="24" height="25" viewBox="0 0 24 25">
                                <path d="M3.516 20.985C2.36988 19.878 1.45569 18.5539..." fill="#C7C7C7" />
                            </svg>
                        </div>
                        ...
                        <section>
                            <Checkbox.Group
                                options={teamOptions.map((team) => ({
                                    label: (
                                        <span className={lockedTeams.includes(team.value) ? "text-gray-500" : ""}>
                                            {team.label}
                                        </span>
                                    ),
                                    value: team.value,
                                    disabled: lockedTeams.includes(team.value),
                                }))}
                                value={selectedTeams}
                                onChange={handleTeamChange}
                                className="mb-4 flex flex-col gap-3"
