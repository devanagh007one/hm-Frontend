...
const UserManagement = () => {
    ...
    const filteredData = useMemo(() => {
        ...
        if (filter === "active") {
            filtered = users.filter((user) => !user.blocked); // Active if not blocked
        } else if (filter === "inactive") {
            filtered = users.filter((user) => user.blocked); // Inactive if blocked
        }
    ...
    const handleRoleChange = (userId, newRole) => {
        ...
        dispatch(changeUserRole(userId, newRole))
          .then((response) => {
            // If the role change is successful, dispatch the success notification
            dispatch(fetchAllUsers());
            dispatch(showNotification(response.message, "success")); // Use the message from the response
          })
          .catch((error) => {
            dispatch(fetchAllUsers());
            // If there was an error, dispatch the failure notification
            dispatch(showNotification("Failed to change the role", "error"));
                    ...
                    {showImportForm && (
                        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
                            ...
                            <div className="flex justify-between items"> 
                                ...
                                </div>                                <div className="flex justify-around mb-6 text-white">
