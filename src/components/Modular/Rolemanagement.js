...
const UserManagement = () => {
  ...
  const filteredData = useMemo(() => {
      ...
      filtered = filtered.filter(
        (item) =>
          ...
          item.roles.some((role) =>
            role.toLowerCase().includes(specificSearchQuery.toLowerCase())
          )
      );
    }
    ...
    switch (filter) {
      ...
      case "role":
        ...
        filtered.sort((a, b) => {
          return (
            (rolePriority[a.roles[0]] || 99) - (rolePriority[b.roles[0]] || 99)
          );
        });
        break;
  ...
  const handleDownload = () => {
    const selectedRows = filteredData
      ...
      .map(
        ({ _id, __v, passwordChangedAt, macAddresses, editLogs, ...row }) => row
      );
    ...
    },
    {
      ...
      render: (text, record) => {
        const imageUrl = record.image
          ? `${process.env.REACT_APP_STATIC_API_URL}${record.image.replace(
              /^.*happme_adminuser_management/,
              ""
            )}`
      ...
      render: (roles, record) => {
        ...
        return (
          <Select
            ...
            options={filteredRolesOptions.map((role) => ({
              label: role,
              value: role,
            }))}
            value={roles[0]}
        ...
        >
          ...
          <div className="flex gap-6">
            ...
            {showImportForm && (
              ...
              >
                <div
                  className={`rounded-lg shadow-lg w-full max-w-xl p-8 relative ${
                    darkMode
                      ? "bg-[#333333] text-dark"
                      : "bg-[#f1f5f9] text-dark"
                  }`}
