...
const ContentManagement = () => {
    ...
    const handleApprovalAction = (record, status) => {
        ...
        if (challengeName) {
            ...
            dispatch(patchTheContent(_id, status))
                .then(() => {
                    dispatch(showNotification(`Successfully updated content status to ${status}`, "success"));
                    dispatch(fetchAllContent());
                })
                .catch(() => {
                    dispatch(showNotification(`Failed to update content status to ${status}`, "error"));
                });
        }
    };
    ...
    const handleDeleteAction = (record) => {
        ...
        if (challengeName) {
            dispatch(deleteChallenge(_id))
                .then(() => {
                    dispatch(showNotification(`Successfully To Delete challenge`, "success"));
                    dispatch(fetchAllContent());
                })
                .catch(() => {
            ...
            dispatch(deleteContent(_id))
                .then(() => {
                    dispatch(showNotification(`Successfully Delete content`, "success"));
                    dispatch(fetchAllContent());
                })
                .catch(() => {
    ...
    const filteredData = useMemo(() => {
        const { challenges = [], modules = [] } = contents.data || {}; // Default to empty arrays


        let combinedData = [];
        let unmatchedChallenges = [];  // Array to store unmatched challenges
        let unmatchedModules = [];  // Array to store unmatched modules

        challenges.forEach((challenge, index) => {
            const matchingModule = modules[index];  // Find matching module for each challenge

            if (matchingModule) {
                combinedData.push({ ...challenge });  // Add challenge
                combinedData.push({ ...matchingModule });  // Add module
            } else {
        ...
        combinedData = [...combinedData, ...unmatchedChallenges, ...unmatchedModules];
    ...
    // Ensure contents.data is an object with challenges and modules arrays
    const cardData = [
        {
            title: "Total Content",
            value: (
    ...
    const columns = [
        {
            ...
            render: (text, record) => record.uniqueUploadId || record.uniChallengeId,
        },
        {
            title: (
                <div className="flex items-center">
        ...
        },
        {
            ...
            key: "moduleType",
            render: (text, record) => record.moduleType || "Challenge",
        },
        {
            title: (
                <div className="flex items-center">
        ...
        },
        {
            ...
            render: (isApproved, record) => {
                ...
                const status = statusMap[isApproved?.toLowerCase()] || "Inactive";
        ...
        },
        {
            ...
            key: "tracks",
            render: (text, record) => record.tracks || record.module?.tracks,
        },
        {
            title: (
                <div className="flex items-center">
