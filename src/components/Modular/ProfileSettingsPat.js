...
const Profilecreads = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Reset edit status BEFORE fetching users
      handleCancel();
      ...
      dispatch(showNotification(`Error updating profile`, "error"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCancel = () => {
    // Reset all edit statuses to false
    const resetEditStatus = {};
    Object.keys(editStatus).forEach((key) => {
      resetEditStatus[key] = false;
    });
    setEditStatus(resetEditStatus);

    // Reset userDetails to current userData
    if (userData) {
      setUserDetails({
        firstName: userData.firstName || "",
        lastName: userData.lastName || "",
  ...
  return (
    <div
      className={`flex mr-3 justify-between items-center ${
    ...
    >
      <section className="w-full flex flex-col justify-center mt-10 items-center">
        <div className="flex w-3/4">
          <img
            src={`${process.env.REACT_APP_STATIC_API_URL}/${
              userData?.image?.replace(/^.*\/uploads\//, "uploads/") ||
              "default.png"
            }`}
          ...
          >
            <div className="w-full flex flex-col gap-5 overflow-y-scroll h-[500px]">
              ...
              <div className="flex justify-end">
                ...
                >
                  <path
                    d="M3 17.25V21H6.75L17.81 9.93997L14.06 6.18997L3 17.25ZM21.41 6.34003C21.79 5.96003 21.79 5.34003 21.41 4.96003L19.04 2.59003C18.66 2.21003 18.04 2.21003 17.66 2.59003L15.87 4.37003L19.63 8.13003L21.41 6.34003Z"
                    fill="gray"
                  />
                </svg>
              </div>
        ...
        {Object.values(editStatus).some((status) => status === true) && (
          <div className="flex gap-5 w-full items-center justify-center mt-20">
            <button
              className="w-28 p-3 rounded-2xl bg-[#F48567] text-white text-lg subpixel-antialiased disabled:opacity-50"
              onClick={handleSave}
              disabled={isLoading}
            >
            ...
            <button
              className="w-28 p-3 rounded-2xl bg-[#C7C7C7] text-white text-lg subpixel-antialiased disabled:opacity-50"
